...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") 
    ...
    DiffSteerModel diffSteerModel;

    auto onWheelSpeedRequest{[&diffSteerModel, &INPUT_ID](
        cluon::data::Envelope &&envelope)
      {
        uint32_t const senderStamp = envelope.senderStamp();
        if (senderStamp == 0) {
          auto leftWheelSpeedRequest = 
            cluon::extractMessage<opendlv::proxy::WheelSpeedRequest>(
                std::move(envelope));
          diffSteerModel.setLeftWheelSpeed(leftWheelSpeedRequest);
        }
        else if (senderStamp == 1) {
          auto rightWheelSpeedRequest = 
            cluon::extractMessage<opendlv::proxy::WheelSpeedRequest>(
                std::move(envelope));
          diffSteerModel.setRightWheelSpeed(rightWheelSpeedRequest);
        }
      }};
    ...
    auto atFrequency{[&FRAME_ID, &VERBOSE, &DT, &diffSteerModel, &od4]() 
      -> bool
      {
        opendlv::sim::KinematicState kinematicState = diffSteerModel.step(DT);
